...
const officialMonitorClient = fs.readFileSync(path.join(root, 'official-monitor.js'), 'utf8');
const officialSearchClient = fs.readFileSync(path.join(root, 'official-search.js'), 'utf8');
...
assert.match(officialMonitorClient, /job-tracker-official-batch/);
assert.match(officialMonitorClient, /while\(typeof localDataReady!==/);
assert.match(officialMonitorClient, /官网巡检确认：流程已结束/);
assert.match(officialMonitorClient, /确认前不会修改投递状态/);
assert.match(officialMonitorClient, /job-tracker-official-inspector\.zip/);
assert.match(onlineServer, /'\.zip':'application\/zip'/);
assert.match(officialSearchClient, /window\.getOfficialInspectionTargets/);
assert.match(officialSearchClient, /!\['未通过','已放弃','已结束'\]\.includes\(application\.status\)/);
assert.match(officialSearchClient, /onclick="startApplicationInspection\(\)"/);
assert.ok(!extensionManifest.permissions.includes('activeTab'));
assert.ok(extensionManifest.optional_host_permissions.includes('https://*/*'));
assert.ok(!extensionManifest.permissions.includes('cookies'));
assert.match(extensionPopup, /chrome\.permissions\.request/);
assert.doesNotMatch(extensionPopup, /document\.cookie|input\.value|body\.innerText/);
assert.match(extensionBackground, /body\.slice\(0,1800\)/);
assert.match(extensionBackground, /evidence:index>=0\?body\.slice/);
assert.doesNotMatch(extensionBackground, /slice\(0,180000\)|document\.cookie|input\.value/);
assert.match(extensionBackground, /chrome\.permissions\.remove/);

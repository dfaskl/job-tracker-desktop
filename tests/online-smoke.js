...
assert.match(reviewReminderClient, /state\.applicationReviewChecks\?\.\[applicationId\]\|\|state\.officialReviewConfirmedAt/);
assert.match(reviewReminderClient, /state\.applicationReviewChecks\[application\.id\]=checkedAt/);
assert.match(reviewReminderClient, /state\.applicationReviewBatchDate===localDateKey\(\)/);
assert.match(reviewReminderClient, /state\.applicationReviewBatchDate=localDateKey\(\)/);
assert.match(reviewReminderClient, /已确认 \$\{count\} 条投递，这些岗位三天后再提醒/);

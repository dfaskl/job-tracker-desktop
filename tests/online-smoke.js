...
const locationPickerClient = fs.readFileSync(path.join(root, 'location-picker.js'), 'utf8');
...
assert.match(locationPickerClient, /'全国'/);
assert.match(locationPickerClient, /'北京市'/);
...
assert.match(scheduleClient, /function calendarRangePosition\(event,date\)/);
assert.match(scheduleClient, /range-middle/);
assert.match(scheduleClient, /calendarRangePosition\(item,date\)!=='middle'/);
assert.doesNotMatch(indexHtml, /review-reminder/);
assert.doesNotMatch(commercialClient, /applicationLeaderboard|投递动态/);

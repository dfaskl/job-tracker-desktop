...
设置页只保留一个“导入数据”入口，会自动校验并识别以下三种 JSON 格式：

- 线上完整备份：同时恢复投递、日程和官网库。
- 旧版 `job-tracker.json`：导入投递记录和日程，替换当前账号已有的投递与日程。
- 旧版 `company-links.json`：合并导入公司官网库；同名公司使用导入文件中的链接。

线上“导出数据”会把投递、日程和官网库合并到一个完整备份文件，恢复该文件时两类业务数据会一并恢复。备份不包含 API 地址、模型或 API Key；API 配置只能在设置页面手动填写，API Key 通过 HTTPS 发送到服务端并使用 AES-256-GCM 加密保存。

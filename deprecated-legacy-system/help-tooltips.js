(function () {
  const pageHelp = {
    ...
    applications: '未来日程优先，其次依次为面试进展、测评或笔试进展、仅投递；未通过排在最后。',
    calendar: '日程可设置为时间点或时间段；时间段在完成后以实际点击完成的时间记录进度。不会发送系统通知。',
    mail: '粘贴邮件正文，由大模型提取岗位和日程；支持时间点或时间段，确认后再保存。',
  ...
  const sectionHelp = {
    ...
    '人工确认': `仅展示面试结束达到${PROGRESS_STALE_DAYS}天仍无进展且未结束的岗位，由你决定是否标记未通过。`,
    '阶段分布': `按当前分类统计；面试结束达到${PROGRESS_STALE_DAYS}天仍无进展的岗位归入“无消息”。`,
    '渠道分布': '按官网、Boss直聘、内推等投递渠道统计。',
    '求职转化漏斗': '累计统计至少到达各阶段的岗位，并计算相对上一阶段的比例。',
    '邮件正文': '粘贴完整通知正文，支持长文本。',
    '识别结果': '可修改识别内容、选择时间点或时间段，并关联已有岗位或新建投递。',
  ...
  const statHelp = {
    ...
    '长期无消息': `最后一次已完成面试距今达到${PROGRESS_STALE_DAYS}天，且仍未结束的岗位。`
  };

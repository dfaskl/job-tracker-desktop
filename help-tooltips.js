(function () {
  const pageHelp = {
    home: '首页是每日工作台：顶部汇总全部投递、进行中岗位、待完成日程和 Offer；中间展示近期日程；底部“人工确认”收录达到10天没有实际进展的岗位，由你决定是否标记为未通过。',
    applications: '集中查看所有岗位及当前阶段、状态和健康度。有后续进展的岗位在前，并按最近一次实际进展时间排序；只有投递、没有后续环节的岗位靠后；未通过统一放在最末尾。新建时若检测到相同公司和岗位，会引导你向原岗位追加日程，避免重复记录。',
    ...
    stats: '查看投递阶段与渠道分布。“无消息”指最近一次实际进展距今达到7天；它会替代原阶段参与统计，避免同一岗位重复计数。',
  ...
  const sectionHelp = {
    ...
    '阶段分布': '只统计已投递、测评、笔试、面试、Offer、已结束和无消息。达到7天无实际进展的岗位归入“无消息”，不再重复计入原阶段。',

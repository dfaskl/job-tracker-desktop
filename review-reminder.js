(function(){
  ...
  function actionHtml(application){
    const url=window.applicationOfficialReviewUrl?.(application.id)||'';
    return url?`<a class="ghost review-reminder-link" href="${esc(url)}" target="_blank" rel="noopener noreferrer">打开投递页 ↗</a>`:`<button class="ghost" type="button" onclick="openDetail('${application.id}')">查看投递详情</button>`;
  }
  function reminderHtml(){
    const queue=dailyQueue();currentReviewItem=queue.items[0]||null;
    if(!currentReviewItem)return'';
    return `<section class="panel review-reminder" role="status"><div class="review-reminder-icon" aria-hidden="true">✓</div><div class="review-reminder-copy"><small>今日进展确认</small><strong>今天需要检查 ${queue.total} 个，还剩 ${queue.items.length} 个</strong><p>每次只处理一个；打开对应投递页面查看进展后，再确认这条已经检查。</p><div class="review-reminder-current"><b>${esc(currentReviewItem.company)}</b><span>${esc(currentReviewItem.position)}</span></div></div><div class="review-reminder-actions">${actionHtml(currentReviewItem)}<button class="primary" type="button" onclick="confirmCurrentApplicationReview('${currentReviewItem.id}')">这条已检查</button></div></section>`;
  }

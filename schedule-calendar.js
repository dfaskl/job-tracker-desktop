(function () {
  ...
  function calendarRangePosition(event,date){if(!eventHasRange(event)||event.completed)return'point';const start=String(event.startsAt||'').slice(0,10),end=String(event.endsAt||'').slice(0,10);if(start===end)return'single';if(date===start)return'start';if(date===end)return'end';return'middle'}
  function calendarChip(event,date){const position=calendarRangePosition(event,date),tone=`chip-${eventTypeTone(event.type)}`,stateClass=`${event.completed?'done':''} ${event.missed?'missed':''}`;if(position==='middle')return `<i class="event-chip ${tone} range-middle" title="${esc(event.company)} · ${esc(event.title||event.type)}（时间段进行中）" aria-label="${esc(event.company)} · ${esc(event.title||event.type)}，时间段进行中"></i>`;return `<i class="event-chip ${tone} ${stateClass} ${position==='start'?'range-start':position==='end'?'range-end':''}"><em>${esc(calendarTime(event,date))}</em>${esc(event.company)} · ${esc(event.title||event.type)}</i>`}
  function scheduleBounds() {
    ...
    return {first:toDate(months[0]),last:toDate(months[months.length-1])};
  }
  function clampScheduleMonth(candidate) {
    const {first,last}=scheduleBounds(),value=monthValue(candidate);
    if(value<monthValue(first))return first;
  ...
  function renderScheduleCalendar(){
    scheduleMonth=clampScheduleMonth(scheduleMonth);
    const year=scheduleMonth.getFullYear(),month=scheduleMonth.getMonth();
    const {first,last}=scheduleBounds(),currentValue=monthValue(scheduleMonth),now=new Date(),thisMonth=new Date(now.getFullYear(),now.getMonth(),1),thisMonthInRange=monthValue(thisMonth)>=monthValue(first)&&monthValue(thisMonth)<=monthValue(last);
    ...
    for(let index=0;index<42;index++){
      ...
      const date=key(year,month,day),items=eventsOn(date),pending=items.filter(item=>!item.completed&&calendarRangePosition(item,date)!=='middle').length,weekend=new Date(year,month,day).getDay()%6===0,past=date<today;
      cells.push(`<button class="schedule-day ${items.length?'has-events':''} ${date===today?'today':''} ${date===selectedDate?'selected':''} ${weekend?'weekend':''} ${past?'past':''}" onclick="selectScheduleDay('${date}',this)"><span class="schedule-day-number">${day}</span><div class="schedule-day-events">${items.slice(0,3).map(item=>calendarChip(item,date)).join('')}${items.length>3?'<small class="more-ellipsis">•••</small>':''}</div>${pending?`<b class="pending-count">${pending}</b>`:''}</button>`);
    }

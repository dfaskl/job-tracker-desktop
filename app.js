...
function load(){try{const cached=JSON.parse(localStorage.getItem(KEY)||'{}');if(cached.settings)delete cached.settings.apiKey;localStorage.setItem(KEY,JSON.stringify(cached));return Object.assign(initial(),cached)}catch{return initial()}}
function save(){
...
function applicationFlowNodes(a){
  ...
  const last=nodes[nodes.length-1],terminal=a.status==='未通过'||a.status==='已放弃'||a.status==='已结束',offer=a.stage==='Offer'||a.status==='已通过';
  let statusLabel=offer?'Offer':terminal?a.status:a.status;
  if(statusLabel&&statusLabel!=='进行中'&&statusLabel!==last.label){const [style,icon]=visual(statusLabel);nodes.push({label:statusLabel,date:'当前状态',kind:offer?'success':terminal?'failed':'current',style,icon});}
...
async function recognizeMail(){const body=document.querySelector('#mailBody').value.trim(),btn=document.querySelector('#recognizeBtn');if(!body)return toast('请先粘贴邮件正文');btn.disabled=true;btn.textContent='正在识别…';try{const res=await fetch('/api/recognize',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...state.settings,body})}),data=await res.json();if(!res.ok)throw new Error(data.error||'识别失败');mailResult=data;document.querySelector('#recognition').innerHTML=mailResultHtml();toast('识别完成')}catch(e){toast('识别失败：'+e.message,{type:'error',duration:5000})}finally{btn.disabled=false;btn.textContent='✦ 使用大模型识别'}}

...
function applicationFlow(a){
  ...
  const nodes=[{label:'已投递',date:a.appliedDate||'',kind:events.length||a.stage!=='已投递'?'done':'current'}];
  events.forEach(e=>{const eventTime=new Date(String(e.startsAt||'').replace(' ','T')).getTime(),kind=e.missed?'failed':e.completed?'done':eventTime>Date.now()?'upcoming':'current';nodes.push({label:e.title||e.type,date:String(e.startsAt||'').slice(0,10),kind});});
  ...
  if(statusLabel&&statusLabel!=='进行中'&&statusLabel!==last.label)nodes.push({label:statusLabel,date:'当前状态',kind:offer?'success':terminal?'failed':'current'});
  else if(!events.length&&a.stage!=='已投递')nodes.push({label:a.stage,date:'当前阶段',kind:'current'});
  return `<div class="application-flow" aria-label="投递流程">${nodes.map((node,index)=>`${index?'<span class="flow-line" aria-hidden="true"></span>':''}<div class="flow-node ${node.kind}"><i>${node.kind==='done'||node.kind==='success'?'✓':node.kind==='failed'?'×':''}</i><b>${esc(node.label)}</b><small>${esc(node.date)}</small></div>`).join('')}</div>`;
}
function appCard(a){return `<div class="application-card application-card-flow" onclick="openDetail('${a.id}')"><div class="card-overview"><div class="card-summary-line"><strong title="${esc(a.company)}">${esc(a.company)}</strong><i>·</i><span class="card-position" title="${esc(a.position)}">${esc(a.position)}</span><i>·</i><span>${esc(a.city||'地点未填')}</span><i>·</i><span>${esc(a.channel||'渠道未填')}</span></div><div class="card-health">${healthBadge(a)}</div></div>${applicationFlow(a)}</div>`}

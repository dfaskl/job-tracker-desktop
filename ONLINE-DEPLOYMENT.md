...
线上版设置页提供“旧版数据迁移”，可分别选择桌面版 `data` 文件夹中的文件：

- `job-tracker.json`：导入投递记录和日程，替换当前账号已有的投递与日程。
- `company-links.json`：合并导入公司官网库；同名公司使用导入文件中的链接。
- `local-config.json`：导入 API 配置；API Key 通过 HTTPS 发送到服务端并立即使用 AES-256-GCM 加密。

线上“导出数据”生成的完整备份同时包含投递、日程和官网库，但不会包含 API Key。恢复完整备份时官网库也会一并恢复，API Key 仍需通过设置表单或 `local-config.json` 单独配置。

...
<style scoped>
.settings-grid{display:grid;height:100%;min-height:0;grid-template-columns:minmax(250px,.82fr) minmax(440px,1.28fr) minmax(380px,1fr);grid-template-rows:auto minmax(0,1fr);grid-template-areas:"official api backup" "about api backup";align-items:stretch;gap:16px;padding:16px 0 18px;box-sizing:border-box;overflow:hidden}
...
@media(max-width:1200px){.settings-grid{height:auto;grid-template-columns:1fr;grid-template-rows:auto;grid-template-areas:"about" "official" "api" "backup";overflow:visible}.settings-grid :deep(.backup-card){max-height:720px}}
</style>
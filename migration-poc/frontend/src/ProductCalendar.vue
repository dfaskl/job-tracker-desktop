      ...
      <section class="card month-card">
        ...
        <div class="month-grid">
          <button v-for="cell in cells" :key="cell.key" :class="['day',{muted:!cell.current,selected:cell.key===selectedDate,today:cell.key===key(new Date())}]" @click="selectedDate=cell.key">
            <span>{{ cell.day }}</span><i v-for="event in cell.events.slice(0,2)" :key="event.id" :class="{ completed:Boolean(event.completed) }">{{ eventName(event) }}</i><small v-if="cell.events.length>2">+{{ cell.events.length-2 }}</small>
          </button>
        </div>
      </section>

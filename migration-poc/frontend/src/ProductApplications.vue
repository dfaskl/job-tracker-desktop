...
<div v-if="selected&&!editing&&!eventEditor" class="backdrop" @click.self="selected=null">
  <section class="modal"><button class="close" @click="selected=null">×</button><div class="detail-head"><div><h2>{{text(selected.company)}}</h2><p>{{text(selected.position)}} · {{text(selected.city,'地点未填')}}</p></div><div class="badges"><b>{{selected.stage}}</b><i>{{selected.status}}</i></div></div>
    <div class="actions"><a class="official" :href="officialUrl(selected)" target="_blank" rel="noreferrer" title="打开招聘官网">官网 ↗</a><button v-if="!store.readOnly.value" :disabled="busy" @click="openEdit(selected)">编辑</button><button v-if="!store.readOnly.value" class="secondary" :disabled="busy" @click="openEvent">日程</button><button v-if="!store.readOnly.value" class="offer" :disabled="busy" @click="quickUpdate('Offer','已通过')">Offer</button><button v-if="!store.readOnly.value" class="abandon" :disabled="busy" @click="quickUpdate('已结束','已放弃')">放弃</button><button v-if="!store.readOnly.value" class="reject" :disabled="busy" @click="quickUpdate('已结束','未通过')">未通过</button><button v-if="!store.readOnly.value" class="danger-button" :disabled="busy" @click="removeApplication">删除</button></div>

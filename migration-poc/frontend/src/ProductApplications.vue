  ...
  <section class="modal detail-modal" role="dialog" aria-modal="true" aria-labelledby="application-detail-title">
    ...
    <div class="detail-company-row">
      <div class="company-identity">
        <div class="company-name-line"><a :href="officialUrl(selected)" target="_blank" rel="noreferrer" title="打开招聘官网"><span class="company-icon" aria-hidden="true">◎</span><strong>{{text(selected.company)}}</strong><span class="company-link-arrow" aria-hidden="true">↗</span></a></div>
...
<div v-if="editing" class="backdrop"><form class="modal form" role="dialog" aria-modal="true" aria-labelledby="application-editor-title" @submit.prevent="saveApplication"><button type="button" class="close" aria-label="关闭" @click="closeEditors">×</button><h2 id="application-editor-title">{{selected?'编辑投递':'新建投递'}}</h2>

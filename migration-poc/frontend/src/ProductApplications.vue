  ...
  <div class="application-toolbar-portal">
    ...
    <div v-if="store.user.value" class="toolbar">
      ...
      <aside class="application-heatmap" aria-label="月度投递数量热力图">
        ...
        <div class="heatmap-controls"><strong>{{ applicationHeatTitle }}</strong><div><button type="button" :disabled="!canHeatPrevious" aria-label="上一个月份" @click="moveApplicationHeatMonth(-1)">‹</button><button type="button" :disabled="!canHeatNext" aria-label="下一个月份" @click="moveApplicationHeatMonth(1)">›</button></div></div>
      </aside>
    </div>

...
const adviceTimeline = computed(() => {
  let previousDate=''
  return (scheduleAdvice.value?.plans||[]).map((text,index)=>{
    const match=text.match(/^(\d{4}-\d{2}-\d{2})\s+(\d{2}:\d{2})-(\d{2}:\d{2})\s+(.+)$/)
    const date=match?.[1]||'',showDate=Boolean(date&&date!==previousDate)
    if(date)previousDate=date
    return {id:index+'-'+text,date,start:match?.[2]||'',end:match?.[3]||'',label:match?.[4]||text,showDate}
  })
})
...
function adviceCacheKey(){return 'job_tracker_schedule_advice_v5_'+String(store.user.value?.email||'guest').toLowerCase()}
...
function localScheduleAdvice():ScheduleAdvice{
  ...
  try{
    ...
    if(!status.callsEnabled){adviceNotice.value=status.message||'请先在设置中完成大模型 API 配置';return}
...
<template>
  <div v-if="store.user.value" class="home-dashboard">
    ...
    <section class="dashboard-panel">
      ...
      <section v-if="adviceLoading || scheduleAdvice || (adviceCandidates.length>1 && adviceNotice)" class="schedule-advice"><div class="advice-head"><div class="advice-title"><i>✦</i><div><strong>安排建议</strong><small>{{adviceLoading?'正在根据时间生成建议…':scheduleAdvice?.summary}}</small></div></div><button class="advice-refresh" :disabled="adviceLoading" @click="generateScheduleAdvice(adviceSignature,0,true)">{{adviceLoading?'生成中…':'重新生成'}}</button></div><p v-if="adviceNotice && !adviceLoading" class="advice-notice">{{adviceNotice}}</p><template v-if="scheduleAdvice"><div v-if="scheduleAdvice.plans?.length" class="advice-content"><ol><li v-for="item in scheduleAdvice.plans" :key="item">{{item}}</li></ol><aside class="advice-timeline" aria-label="日程时间轴"><strong>时间轴</strong><div class="timeline-list"><article v-for="item in adviceTimeline" :key="item.id"><time>{{item.showDate?item.date.slice(5).replace('-','月')+'日':''}}</time><i></i><span><b v-if="item.start">{{item.start}}–{{item.end}}</b><em>{{item.label}}</em></span></article></div></aside></div><div v-if="scheduleAdvice.conflicts?.length" class="advice-conflicts"><strong>时间冲突</strong><span v-for="item in scheduleAdvice.conflicts" :key="item">{{item}}</span></div></template></section>

...
const adviceTimeline = computed(() => {
  const groups = new Map<string,{id:string,date:string,start:string,end:string,labels:string[]}>()
  for(const text of scheduleAdvice.value?.plans||[]){
    const match=text.match(/^(\d{4}-\d{2}-\d{2})\s+(\d{2}:\d{2})-(\d{2}:\d{2})\s+(.+)$/)
    const date=match?.[1]||'',start=match?.[2]||'',end=match?.[3]||'',label=match?.[4]||text
    const key=date+' '+start
    const group=groups.get(key)
    if(group)group.labels.push(label)
    else groups.set(key,{id:key||text,date,start,end,labels:[label]})
  }

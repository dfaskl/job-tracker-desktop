...
const adviceTimeline = computed(() => {
  ...
  if(entries.length){
    for(const entry of entries){
      ...
      const windowStart=entry.windowStart?.slice(11,16)||entry.start, windowEnd=entry.windowEnd?.slice(11,16)||entry.end
      const timeLabel=entry.showWindow?windowStart+'–'+windowEnd:entry.start
...
function adviceCacheKey(){return 'job_tracker_schedule_advice_v8_'+String(store.user.value?.email||'guest').toLowerCase()}

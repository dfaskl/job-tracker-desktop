...
type MigrationResult={ok:boolean;scannedUsers:number;changedUsers:number;migratedEvents:number;skippedEvents:number}
...
const actionLabels:Record<string,string>={'disable-user':'停用账号','enable-user':'启用账号','delete-user':'删除账号','open-registration':'开放注册','close-registration':'关闭注册','set-registration-code':'设置注册码','clear-registration-code':'清除注册码','view-user-details':'查看用户详情','revoke-sessions':'撤销登录会话','migrate-completed-ranges':'迁移旧时间段日程'}
...
async function saveRegistrationCode(){const code=registrationCode.value.trim();if(code.length<4){error.value='注册码至少需要 4 位';return}loading.value=true;error.value='';try{await requestJson('/api/poc/admin-sandbox/settings/registration-code',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({code,clear:false})});registrationCode.value='';await loadOverview();message.value='注册码已安全更新'}catch(cause){error.value=failure(cause,'更新注册码失败')}finally{loading.value=false}}
async function clearRegistrationCode(){if(!confirm('清除后，新用户注册将不再要求注册码。确认继续吗？'))return;loading.value=true;error.value='';try{await requestJson('/api/poc/admin-sandbox/settings/registration-code',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({clear:true})});registrationCode.value='';await loadOverview();message.value='注册码已清除'}catch(cause){error.value=failure(cause,'清除注册码失败')}finally{loading.value=false}}
...
async function migrateCompletedRanges(){const confirmation=prompt('该操作会修改所有用户的旧时间段日程，并为每位受影响用户自动创建备份。\n请输入“迁移旧日程”确认：','');if(confirmation===null)return;if(confirmation!=='迁移旧日程'){error.value='确认文字不匹配，未执行迁移';return}loading.value=true;error.value='';message.value='';try{const result=await requestJson('/api/poc/admin-sandbox/maintenance/migrate-completed-ranges',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({confirmation})}) as MigrationResult;await loadOverview();message.value=`迁移完成：扫描 ${result.scannedUsers} 个用户，更新 ${result.changedUsers} 个用户的 ${result.migratedEvents} 项日程${result.skippedEvents?`，跳过 ${result.skippedEvents} 项缺少有效完成时间的记录`:''}`}catch(cause){error.value=failure(cause,'旧日程迁移失败')}finally{loading.value=false}}
...
<template>
  <section class="admin-shell">
      ...
      <div class="admin-grid">
        ...
        <article><i>AI 配置</i><strong>{{overview.summary.configuredApiKeys}}</strong><small>已配置独立密钥</small></article>
          ...
          <section class="card control-card">
            ...
            <div class="migration-tool"><div><strong>旧日程一次性迁移</strong><small>将已完成的旧时间段收束为完成时间点；修改前按用户自动备份，缺少完成时间的记录会跳过。</small></div><button class="migration-button" :disabled="loading" @click="migrateCompletedRanges">{{loading?'处理中…':'迁移全部用户'}}</button></div>
          </section>
        ...
        <section class="card users-card">
          ...
          <div class="user-tools"><input v-model="query" type="search" aria-label="搜索用户邮箱" placeholder="搜索用户邮箱" /><BaseSelect v-model="stateFilter" :options="[{value:'all',label:'全部状态'},{value:'enabled',label:'正常'},{value:'disabled',label:'已停用'},{value:'admin',label:'管理员'}]" /></div>
    ...
    <div v-if="selected" class="modal-backdrop" @click.self="selected=null">
      <section class="detail-modal" role="dialog" aria-modal="true" aria-labelledby="admin-user-detail-title"><button class="modal-close" aria-label="关闭" @click="selected=null">×</button><header><div class="avatar large">{{selected.email.slice(0,1).toUpperCase()}}</div><div><span>用户详情</span><h2 id="admin-user-detail-title">{{selected.email}}</h2><p>{{selected.applicationCount}} 条投递 · {{selected.eventCount}} 项日程 · API Key {{selected.hasApiKey?'已配置':'未配置'}}</p></div></header><div v-if="!detail" class="loading-panel">正在读取详情…</div><div v-else class="detail-scroll"><p v-if="detail.truncated" class="hint">共 {{detail.totalApplications}} 条投递，当前展示前 500 条。</p><article v-for="application in detail.applications" :key="application.id"><div><strong>{{application.company||'未填写公司'}} · {{application.position||'未填写岗位'}}</strong><span>{{application.stage||'—'}} / {{application.status||'—'}} · {{application.city||'地点未填'}} · {{application.channel||'渠道未填'}}</span></div><ol><li v-for="step in application.flow" :key="step.at+step.title"><time>{{step.at||'时间未知'}}</time><span>{{step.title}}</span></li></ol></article><p v-if="!detail.applications.length" class="empty">该用户暂无投递记录</p></div></section>
    </div>
  </section>
</template>

<style scoped>
.admin-shell{display:flex;height:100%;min-height:0;flex-direction:column;gap:18px;padding:18px 0;overflow:hidden}.admin-head,.section-title,.registration,.user-tools,.users-card article,.actions,.detail-modal header{display:flex;align-items:center;justify-content:space-between;gap:12px}.admin-head h2,.admin-head p,.section-title h3,.detail-modal h2,.detail-modal p{margin:0}.admin-head>div>span,.section-title>div>span{color:var(--accent,var(--color-primary));font-size:11px;font-weight:800;letter-spacing:.09em}.admin-head h2{margin:3px 0;font-size:22px}.admin-head p{color:var(--color-muted-foreground);font-size:12px}.summary-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.summary-grid article{display:grid;gap:4px;padding:13px 16px;border:1px solid #dfe5ef;border-radius:12px;background:#fff}.summary-grid i{color:var(--color-muted-foreground);font-size:11px;font-style:normal}.summary-grid strong{font-size:24px}.summary-grid small{color:#7a8699}.admin-grid{display:grid;min-height:0;flex:1;grid-template-columns:minmax(300px,.9fr) minmax(620px,1.8fr) minmax(300px,.9fr);gap:18px}.overview-column{display:flex;min-height:0;flex-direction:column;gap:14px}.overview-column .control-card{min-height:0;flex:1}.card{min-width:0;margin:0;padding:17px;border-radius:14px}.section-title h3{margin-top:3px;font-size:17px}.section-title>small{color:var(--color-muted-foreground)}.section-title>b{padding:5px 8px;border-radius:999px;font-size:11px}.section-title>b.ok{color:#147847;background:#e8f7ee}.section-title>b.muted{color:#83591a;background:#fff2d7}.registration{margin-top:14px;padding:12px;border-radius:10px;background:#f6f8fb}.registration>div{display:grid;gap:3px}.registration small{color:var(--color-muted-foreground)}.registration button{padding:8px 12px}.danger-outline{border:1px solid #efc4c0;color:#a63731;background:#fff}.health-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:11px}.health-grid>div{display:grid;grid-template-columns:10px 1fr auto;align-items:center;gap:7px;padding:9px;border:1px solid #e7ebf2;border-radius:9px}.health-grid span,.health-grid b{font-size:11px}.health-grid b{color:var(--color-muted-foreground)}.dot{width:8px;height:8px;border-radius:50%}.dot.ok{background:#32a66a}.dot.warn{background:#e2a336}.audit-card,.users-card{display:flex;min-height:0;flex-direction:column}.audit-scroll,.user-scroll,.detail-scroll{min-height:0;overflow:auto;scrollbar-width:thin}.audit-scroll{margin-top:10px}.audit-scroll>div{display:grid;grid-template-columns:8px 1fr auto;align-items:center;gap:8px;padding:10px 2px;border-top:1px solid #edf0f5}.audit-scroll>div>i{width:7px;height:7px;border-radius:50%;background:var(--accent,var(--color-primary))}.audit-scroll span{display:grid;gap:2px}.audit-scroll small,.audit-scroll time{color:#7b8798;font-size:10px}.user-tools{margin:12px 0}.user-tools input{flex:1}.user-tools .base-select{width:130px;flex:none}.users-card article{padding:11px 4px;border-top:1px solid #e9edf3}.users-card article.disabled{opacity:.65}.avatar{display:grid;width:36px;height:36px;flex:none;place-items:center;border-radius:10px;color:#fff;background:var(--accent,var(--color-primary));font-weight:800}.avatar.large{width:48px;height:48px;border-radius:13px}.identity{display:grid;min-width:0;flex:1;gap:4px}.identity strong{overflow:hidden;text-overflow:ellipsis}.identity span{color:#788496;font-size:11px}.identity span b{color:#6a4a96}.identity span b.good{color:#197348}.identity span b.bad{color:#a63833}.counts{display:flex;min-width:235px;gap:14px;color:#697587;font-size:11px}.counts b{color:#26364e}.actions{justify-content:flex-end}.actions button{padding:7px 9px;font-size:11px}.danger-button{background:#c33b36}.hint,.empty{color:#758195;font-size:11px}.empty{text-align:center}.feedback{position:fixed;top:18px;left:50%;z-index:70;transform:translateX(-50%);padding:11px 18px;border-radius:10px;box-shadow:0 8px 28px rgba(0,0,0,.14)}.modal-backdrop{position:fixed;inset:0;z-index:50;display:grid;place-items:center;padding:24px;background:rgba(18,28,45,.58)}.detail-modal{position:relative;display:flex;width:min(960px,94vw);height:min(760px,88vh);flex-direction:column;padding:24px;border-radius:18px;background:#fff;box-shadow:0 24px 80px rgba(0,0,0,.26)}.detail-modal header{justify-content:flex-start;padding-bottom:16px;border-bottom:1px solid #e7ebf1}.detail-modal header>div:last-child{display:grid;gap:3px}.detail-modal header>div>span{color:var(--accent,var(--color-primary));font-size:11px;font-weight:800}.modal-close{position:absolute;top:16px;right:16px;width:38px;height:38px;padding:0;color:#506078;background:#eef2f7;font-size:22px}.detail-scroll{margin-top:10px}.detail-scroll>article{padding:14px 2px;border-bottom:1px solid #e8edf3}.detail-scroll>article>div{display:grid;gap:4px}.detail-scroll>article>div span{color:#687689;font-size:12px}.detail-scroll ol{margin:10px 0 0;padding-left:20px}.detail-scroll li{display:grid;grid-template-columns:150px 1fr;gap:10px;margin:6px 0;color:#445166}.detail-scroll time{color:#798698;font-size:11px}.disabled-panel,.loading-panel{text-align:center}@media(max-width:1250px){.admin-shell{height:auto;overflow:visible}.admin-grid{grid-template-columns:1fr}.audit-scroll,.user-scroll{max-height:520px}}@media(max-width:720px){.summary-grid{grid-template-columns:1fr 1fr}.users-card article{align-items:flex-start;flex-wrap:wrap}.counts{width:100%;min-width:0;margin-left:48px}.actions{width:100%;margin-left:48px;justify-content:flex-start}.health-grid{grid-template-columns:1fr}.detail-scroll li{grid-template-columns:1fr}}
...
.migration-tool{display:grid;gap:10px;margin-top:10px;padding:12px;border:1px solid #ead8a9;border-radius:10px;background:#fff9e9}.migration-tool>div{display:grid;gap:4px}.migration-tool strong{font-size:12px}.migration-tool small{color:#7b6841;font-size:10px;line-height:1.5}.migration-button{justify-self:start;border-color:#d5a94f;color:#7a5200;background:#fff}
</style>

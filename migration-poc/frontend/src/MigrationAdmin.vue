...
const filteredUsers=computed(()=>{
  ...
  return [...result].sort((a,b)=>{
    ...
    if(sortMode.value==='created-new')return created(b)-created(a)||email(a).localeCompare(email(b),'zh-CN')
    if(sortMode.value==='created-old'){const aTime=created(a),bTime=created(b);return (!aTime?1:!bTime?-1:aTime-bTime)||email(a).localeCompare(email(b),'zh-CN')}
    return Number(!a.groupId)-Number(!b.groupId)||String(a.groupName||'').localeCompare(String(b.groupName||''),'zh-CN')||email(a).localeCompare(email(b),'zh-CN')
  })
})
        ...
        <article><i>AI 配置</i><strong>{{overview.summary.configuredApiKeys}}</strong><small>已配置独立密钥</small></article>
          ...
          <section class="card control-card">
            ...
            <div class="group-manager">
              ...
              <div v-if="overview.groups.length" class="group-list"><span v-for="group in overview.groups" :key="group.id"><b>{{group.name}}</b><small>{{group.memberCount}} 人</small><button class="danger-outline" :disabled="busyUser==='group:'+group.id" :aria-label="'删除小组 '+group.name" @click="deleteGroup(group)">删除</button></span></div>

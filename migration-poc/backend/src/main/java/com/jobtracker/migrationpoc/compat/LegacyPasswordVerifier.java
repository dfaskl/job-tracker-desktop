...
public class LegacyPasswordVerifier {
    ...
    public PasswordRecord create(String password) {
        ...
        byte[] hash=SCrypt.generate(password.getBytes(StandardCharsets.UTF_8),salt.getBytes(StandardCharsets.UTF_8),COST,BLOCK_SIZE,PARALLELIZATION,KEY_LENGTH);
        return new PasswordRecord(salt,HexFormat.of().formatHex(hash));
    }
    public boolean verify(String password,String salt,String expectedHex) {
        ...
        try {
            ...
            byte[] actual=SCrypt.generate(password.getBytes(StandardCharsets.UTF_8),salt.getBytes(StandardCharsets.UTF_8),COST,BLOCK_SIZE,PARALLELIZATION,KEY_LENGTH);
            return MessageDigest.isEqual(actual,expected);
        } catch(IllegalArgumentException exception){return false;}
    }
    public record PasswordRecord(String salt,String hash){}
}

...
public class MailInboxService {
    ...
    private long testConnection(String email,String provider,String password){try(Store store=Session.getInstance(mailProperties()).getStore("imaps")){store.connect(host(provider),993,email,password);Folder folder=store.getFolder("INBOX");folder.open(Folder.READ_ONLY);try{return folder.getMessageCount()>0?((UIDFolder)folder).getUID(folder.getMessage(folder.getMessageCount())):0;}finally{folder.close(false);}}catch(Exception e){throw new ValidationException("邮箱连接失败，请确认已开启 IMAP 并使用授权码");}}

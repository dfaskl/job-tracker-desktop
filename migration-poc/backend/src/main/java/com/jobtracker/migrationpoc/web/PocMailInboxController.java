...
public class PocMailInboxController {
    ...
    @GetMapping("/pending-count") public ResponseEntity<?> pendingCount(@CookieValue(value=PocAuthController.COOKIE_NAME,required=false)String token){return withUser(token,u->Map.of("count",inbox.pendingCount(u.email())));}

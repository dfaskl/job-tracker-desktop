...
public class PocMailInboxController {
    ...
    @PatchMapping("/messages/processed") public ResponseEntity<?> processAll(@CookieValue(value=PocAuthController.COOKIE_NAME,required=false)String token,HttpServletRequest request){if(!sameOrigin(request))return error(HttpStatus.FORBIDDEN,"请求来源无效");return withUser(token,u->Map.of("ok",true,"processed",inbox.processAll(u.email())));}
